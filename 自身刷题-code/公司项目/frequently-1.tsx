...
export default props => {
    ...
    const showVoicePop = (status, cancelStatus, type?, locationY?) => {
        // 根据语音操作的不同阶段（开始、结束、取消）来管理弹窗状态、计时器、以及语音识别行为（开始/结束/取消）。
        if (type == 'new') {
            setTooShort(false)
    ...
    const handleClickSwitchIcon = () => {
        //切换其的发送形式，要不然就是发送语音信息，要不然就是发送文本信息
        if (rtcV3OnlineStatus === 'on') {
            Taro.showToast({
                title: '当前正在通话，请挂断后操作',
                context,
...
//看完，总结亮点：第一，在加入语音聊天之后的eventCenter机制，
// 2.对于按钮按下以及抬起的处理，
//3.对于不同信息的分类，及，这个组件主要处理了两类信息：语音，文本